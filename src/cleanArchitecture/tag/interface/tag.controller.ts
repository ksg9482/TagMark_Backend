...
} from 'src/controllers/dtos/tag/get-search-tags.dto';
...
export class TagController {
  ...
  @Get('/')
  async getUserAllTags(@AuthUser() userId: number) {
  ...
  @Get('/count')
  async getUserTagCount(@AuthUser() userId: number) {
    const getUserAllTagsResponse = new GetUserAllTagsResponseDto();
    try {
      const tags = await this.tagUseCases.getUserAllTags(userId);
  ...
  async detachTag(
    ...
    try {
      const numTagIds = tagIds.split(',').map((strNum) => {
