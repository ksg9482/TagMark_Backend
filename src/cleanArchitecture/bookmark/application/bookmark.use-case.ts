...
import { BookmarkAndTag, BookmarkTagMap } from "src/cleanArchitecture/bookmark/domain/bookmark.interface";
...
export class BookmarkUseCases {
    ...
    async getUserAllBookmarks(userId: string, page: GetUserAllBookmarksDto) {
        ...
        const bookmarks: Page<Bookmark> = await this.bookmarkRepository.getUserAllBookmarks(
            userId,
            {
                take: limit,
                skip: offset
    ...
    async syncBookmark(bookmarks: Bookmark[]) {
        const bookmarkInsert = await this.bookmarkRepository.syncBookmark(bookmarks);
    ...
    async findBookmark(userId: string, bookmarkId: string): Promise<Bookmark> {
        const bookmark = await this.bookmarkRepository.getUserBookmark(userId, bookmarkId);
  ...
  async getTagAllBookmarksOR(
    ...
    const limit = page.getLimit();
    const offset = page.getOffset();
    const bookmarks = await this.bookmarkRepository.findBookmarkTag_OR(userId, tags, {
      take: limit,
      skip: offset,
    });
    return bookmarks;
  ...
  async getTagAllBookmarksAND(
    ...
    const limit = page.getLimit();
    const offset = page.getOffset();
    const bookmarks = await this.bookmarkRepository.findBookmarkTag_AND(userId, tags, {
      take: limit,
      skip: offset,
    });
    return bookmarks;
    ...
    protected async saveBookmarkTag(bookmarks: Bookmark[]) {
        ...
        const result = await this.bookmarkRepository.attachbulk(bookmarksAndTagsMap);
