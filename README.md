...
* nginx
  * WS와 WAS 분리를 해야 더 안전하고 서버의 부담을 덜 수 있다는 것은 알고 있었지만 실제로 적용하지는 않았습니다. 실제로 적용해보려고 도입하였고, 아파치도 선택지에 있었지만 nginx가 성능이 좋고 비동기적이라는 특성때문에 nodejs와 궁합이 좋아 nginx를 선택했습니다.
* docker
...
#### 후기
이 프로젝트는 실제로 제가 사용할 사이트이기 때문에 장기적인 리펙토링, 학습한 내용의 지속적인 적용을 염두에 두었습니다. 객체지향 패러다임과 함수형 패러다임 모두에 관심이 있기 때문에 백엔드에서는 NestJS를 통해 객체지향을 적용하고, 프론트엔드에서는 함수형 패러다임을 적용할 예정입니다.
