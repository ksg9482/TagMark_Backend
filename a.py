...
# 두수의 합
"""
브루트 포스로 계산. 배열을 2번 반복하면서 모든 조합을 더해서 일일이 확인 해보는 무차별 대입 방식. 
시간복잡도 O(n^2)
"""
...
"""
in을 이용한 탐색. 모든 조합을 비교하지 않고 target - n이 존재하는지 탐색
...
"""
첫번째 수를 뺀 결과 키 조회. 타겟에서 첫번째 수를 빼면 두번째 수를 바로 알아낼 수 있다.
...
"""
조회 구조 개선. 딕셔너리 저장과 조회를 2개의 for문으로 각각 처리했던 방식을 좀 더 개선해서 이번에는 하나의 for문으로 합쳐서 처리.
